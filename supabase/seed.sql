-- ============================================================================
-- PolyMRR seed data
-- Run this AFTER the schema migration and AFTER you've signed in at least once
-- (so your profile exists). Replace YOUR_USER_ID below with your actual
-- profile id from the profiles table.
-- ============================================================================

-- Step 0: Set your user id here (run: select id from profiles limit 1;)
...
-- Markets (use fixed UUIDs so bets can reference them)
insert into markets (id, startup_slug, type, question, resolution_criteria, status, yes_odds, total_credits, total_bettors, created_at, closes_at, resolved_at, resolved_outcome) values
  ('a0000000-0000-0000-0000-000000000001', 'shipfast',     'mrr-target',  'Will ShipFast hit $50k MRR by June 2026?',              'TrustMRR verified MRR >= $50,000 on June 30.',          'open',     72, 18500, 42, '2026-01-15', '2026-06-30', null, null),
  ('a0000000-0000-0000-0000-000000000002', 'chatbase',     'mrr-target',  'Will Chatbase hit $100k MRR by April 2026?',             'TrustMRR verified MRR >= $100,000 on April 30.',        'open',     85, 32000, 78, '2026-01-01', '2026-04-30', null, null),
  ('a0000000-0000-0000-0000-000000000003', 'pulsetic',     'acquisition', 'Will Pulsetic be acquired before July 2026?',            'Public announcement of completed acquisition.',          'open',     38, 12000, 31, '2026-02-01', '2026-07-01', null, null),
  ('a0000000-0000-0000-0000-000000000004', 'typingmind',   'growth-race', 'Will TypingMind grow faster than Chatbase this quarter?', 'Compare Q1 2026 growth rates via TrustMRR.',            'open',     45, 22000, 55, '2026-01-01', '2026-04-01', null, null),
  ('a0000000-0000-0000-0000-000000000005', 'draftql',      'survival',    'Will DraftQL still be active in December 2026?',         'TrustMRR shows revenue > $0 in Dec 2026.',              'open',     62,  4500, 15, '2026-02-15', '2026-12-31', null, null),
  ('a0000000-0000-0000-0000-000000000006', 'podsqueeze',   'mrr-target',  'Will PodSqueeze hit $15k MRR by May 2026?',              'TrustMRR verified MRR >= $15,000 on May 31.',           'open',     68,  9800, 28, '2026-01-20', '2026-05-31', null, null),
  ('a0000000-0000-0000-0000-000000000007', 'screenshotai', 'mrr-target',  'Will ScreenshotAI hit $5k MRR by August 2026?',          'TrustMRR verified MRR >= $5,000 on Aug 31.',            'open',     55,  6200, 19, '2026-02-01', '2026-08-31', null, null),
  ('a0000000-0000-0000-0000-000000000008', 'analyzee',     'growth-race', 'Will Analyzee 10x its MRR by end of 2026?',              'TrustMRR verified MRR >= $8,000 on Dec 31.',            'open',     40,  3200, 12, '2026-03-01', '2026-12-31', null, null),
  ('a0000000-0000-0000-0000-000000000009', 'notionforms',  'mrr-target',  'Will NotionForms hit $10k MRR by Q3 2026?',              'TrustMRR verified MRR >= $10,000 on Sep 30.',           'open',     58,  7600, 22, '2026-01-10', '2026-09-30', null, null),
  ('a0000000-0000-0000-0000-000000000010', 'churnkey',     'mrr-target',  'Will Churnkey hit $35k MRR by May 2026?',                'TrustMRR verified MRR >= $35,000 on May 31.',           'open',     70, 14000, 35, '2026-01-05', '2026-05-31', null, null),
  -- 2 resolved markets for history
  ('a0000000-0000-0000-0000-000000000011', 'shipfast',     'mrr-target',  'Did ShipFast hit $40k MRR by Jan 2026?',                 'TrustMRR verified MRR >= $40,000 on Jan 31.',           'resolved', 80, 25000, 60, '2025-07-01', '2026-01-31', '2026-02-01', 'yes'),
  ('a0000000-0000-0000-0000-000000000012', 'chatbase',     'mrr-target',  'Did Chatbase hit $90k MRR by Dec 2025?',                 'TrustMRR verified MRR >= $90,000 on Dec 31.',           'resolved', 65, 20000, 48, '2025-06-01', '2025-12-31', '2026-01-02', 'no');

-- Bets (reference your real user — replace the UUID below)
-- Uncomment and replace YOUR_USER_ID with your actual profile id:
--
-- insert into bets (market_id, user_id, side, amount, odds_at_time, created_at) values
--   ('a0000000-0000-0000-0000-000000000001', 'YOUR_USER_ID', 'yes', 200, 70, '2026-02-10 14:30:00+00'),
--   ('a0000000-0000-0000-0000-000000000002', 'YOUR_USER_ID', 'yes', 500, 82, '2026-02-12 09:15:00+00'),
--   ('a0000000-0000-0000-0000-000000000003', 'YOUR_USER_ID', 'no',  150, 40, '2026-02-20 16:45:00+00'),
--   ('a0000000-0000-0000-0000-000000000004', 'YOUR_USER_ID', 'yes', 300, 48, '2026-02-25 11:00:00+00'),
--   ('a0000000-0000-0000-0000-000000000011', 'YOUR_USER_ID', 'yes', 400, 75, '2025-10-15 13:00:00+00'),
--   ('a0000000-0000-0000-0000-000000000012', 'YOUR_USER_ID', 'yes', 250, 60, '2025-09-20 10:30:00+00');
