-- Track credits bet on each side for amount-proportional payouts
alter table markets add column if not exists total_yes_credits bigint not null default 0;
alter table markets add column if not exists total_no_credits bigint not null default 0;

-- Backfill from existing bets
update markets m set
  total_yes_credits = coalesce((select sum(amount) from bets where market_id = m.id and side = 'yes'), 0),
  total_no_credits  = coalesce((select sum(amount) from bets where market_id = m.id and side = 'no'), 0);

-- Update place_bet to track credits per side
